...
class VidLister {
    /**
     * Constructs the VidLister object
     *
    ...
    function __construct(modX &$modx,array $config = array()) {
        ...
        $basePath = $this->modx->getOption('vidlister.core_path',$config,$this->modx->getOption('core_path').'components/vidlister/');
        $assetsUrl = $this->modx->getOption('vidlister.assets_url',$config,$this->modx->getOption('assets_url').'components/vidlister/');
        ...
        $this->modx->addPackage('vidlister',$this->config['modelPath']);
    }
    ...
    public function initialize($ctx = 'web') {
        switch ($ctx) {
            case 'mgr':
                $this->modx->lexicon->load('vidlister:default');

                if (!$this->modx->loadClass('vidlisterControllerRequest',$this->config['modelPath'].'vidlister/request/',true,true)) {
                    return 'Could not load controller request handler.';
                }
                $this->request = new vidlisterControllerRequest($this);
